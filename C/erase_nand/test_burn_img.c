...
#endif
    int mtd_index = atoi(argv[2]);
    if (0 == mtd_index)
    {
        nRet = erase_block("/dev/mtdblock3",0,0xFFFFFFFF);
    }
    
    if (1 == mtd_index)
    {
        nRet = erase_block("/dev/mtdblock6",0,0xFFFFFFFF);
    }
...
int burn_file_to_mtd(char * file_name,int nType)
{
    ...
    nRet = burn_data(file_name,"/dev/mtdblock4",nType);
...
int burn_data(char * src_name,char * mtd_name,int nType)
{
    ...
    if(2 == nType)
    {
        int nRet = fseek(infile,sizeof(struct img_header),SEEK_SET);
	    if (0 != nRet)
	    {
	    	printf("unable to seek kernel\n");
	    	return -3;
	    }
    }
